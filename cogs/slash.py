...
class Slash(commands.Cog):
    ...
    @slash_command(guild_ids=read_config()["guild_ids"], description="Adds an Up and Downvote Rection to the last message in the Channel")
    ...
    @slash_command(guild_ids=read_config()["guild_ids"], description="Let the universe decide")
    async def coinflip(self, ctx):
        await ctx.respond(f"The universe has made a decision, the coin landed on: `{'Tails' if random.randint(0,1) == 1 else 'Heads'}`")
    
    @slash_command(guild_ids=read_config()["guild_ids"], description="Find your lucky number now (default range is 1-100)")
    ...
    @slash_command(guild_ids=read_config()["guild_ids"], description="Shows you the bots latency to the Discord API")
    async def ping(self, ctx):
        await ctx.respond(f"My Latency is: `{int(round(self.bot.latency*60, 0))}`ms")

    @slash_command(guild_ids=read_config()["guild_ids"], description="Calculates the time simon needs to show up, given the time simon said he would need")
